...
import { fetchPayloadCollection, mapPayloadBlogPost } from "@/lib/cms-client";
...
export default function BlogsPage() {
  ...
  useEffect(() => {
    const loadPosts = async () => {
      const docs = await fetchPayloadCollection("blog-posts", "sort=-publishedAt&depth=1");
      setPosts(docs.map(mapPayloadBlogPost));
    };
  ...
  const filteredPosts = posts.filter((p) => {
    ...
    return p.categories.map((category) => category.toLowerCase()).includes(activeCategory) && !p.featured;
  });
