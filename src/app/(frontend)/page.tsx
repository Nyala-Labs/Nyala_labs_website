...
import type { Activity, BlogPost } from "@/types";
import {
...
export default function HomePage() {
  ...
  useEffect(() => {
    const loadHomeContent = async () => {
      const [postsDocs, activityDocs, homepageData] = await Promise.all([
        fetchPayloadCollection("blog-posts", "limit=3&sort=-publishedAt&depth=1"),
        fetchPayloadCollection("activities", "sort=date&depth=1"),
        fetchHomepage(),
      ]);

      const posts = postsDocs.map(mapPayloadBlogPost);
      ...
      setHighlights(
        homepageData.highlights.length > 0
          ...
          : defaultHighlightCaptions.map((caption) => ({
              image: "",
              caption,
            }))
      );
