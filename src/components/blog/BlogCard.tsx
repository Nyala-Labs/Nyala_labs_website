    ...
    return (
      <Link href={`/blogs/${post.slug}`}>
        {isDesktop ? (
          ...
          <article className="reveal-up group relative cursor-pointer overflow-hidden border border-nyala-gray-light bg-nyala-gray">
            <FeaturedContent post={post} />
          </article>
        )}
      </Link>
  ...
  return (
    <Link href={`/blogs/${post.slug}`}>
      {isDesktop ? (
        ...
        <article
          className="reveal-up group relative cursor-pointer overflow-hidden border border-nyala-gray-light bg-nyala-gray"
...
function FeaturedContent({ post }: { post: BlogPost }) {
  return (
      ...
      <div className="flex flex-col justify-center p-8 md:p-12">
        <div className="mb-3 flex items-center gap-3">
          {post.categories.slice(0, 2).map((cat) => (
            <span key={cat} className="font-mono text-[10px] uppercase tracking-widest text-nyala-yellow">#{cat}</span>
          ))}
        </div>
        ...
        <p className="mt-4 font-mono text-sm leading-relaxed text-nyala-gray-muted">{post.excerpt}</p>
  ...
  return (
    <>
      ...
      <div className="p-6">
        ...
        <p className="mt-2 line-clamp-2 font-mono text-xs leading-relaxed text-nyala-gray-muted">{post.excerpt}</p>
